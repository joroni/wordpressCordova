...
function nonceGet(data){
    $.ajax({ 
        method: 'GET',
        url: 'http://178.128.63.151/bnext2/api/get_nonce/?controller=user&method=generate_auth_cookie', 
       /*  config: {
            headers: {
        ...
        complete: function(data) {
            ...
            var str1 = data.responseText;
            var str2 = "filtering"
            var str3 = str1.replace(str2,"");
            localStorage.setItem("nonceData",str3);
            var authtemp = localStorage.getItem("nonceData");
            var auth_cookieName = localStorage.getItem("auth_cookie_name");
            
                var nonceUser = JSON.parse(authtemp);
                console.log(nonceUser);
                if(nonceUser.status == "ok"  &&  auth_cookieName !== ""){
...
var loginAuth = {
    login:function(loginData){
        $.ajax({url: 'http://178.128.63.151/bnext2/api/user/generate_auth_cookie/?username='+loginCredentials.username+'&password='+loginCredentials.password+'&insecure=cool',
            data: loginData,
            async: true,
            beforeSend: function() {
                $.mobile.loading('show');
